...
var CanonizationClient = class {
  ...
  async canonizeFile(file, stages = ["discovery", "classification", "reconciliation"]) {
...
var IndexConfirmationModal = class extends import_obsidian11.Modal {
...
var IndexProgressModal = class extends import_obsidian11.Modal {
  constructor() {
    super(...arguments);
    this.statusEl = null;
  ...
  complete(stats) {
    ...
    new import_obsidian11.Setting(this.contentEl).addButton((button) => {
...
var FolderSelectionModal = class extends import_obsidian11.Modal {
...
var SemanticAIPlugin = class extends import_obsidian12.Plugin {
  async onload() {
  ...
  registerCommands() {
    this.addCommand({ id: "canonize-paper", name: "Canonize this paper", editorCallback: async (_editor, view) => {
      if (view.file) {
        ...
        new import_obsidian12.Notice(`Created candidate ${record.recordId}; not admitted.`);
      }
    } });
    this.addCommand({ id: "canonize-folder", name: "Canonize this folder", callback: async () => {
      ...
      if (!folder)
        return new import_obsidian12.Notice("Open a note in the folder first.");
      const records = await this.canonizationClient.canonizeFolder(folder);
      new import_obsidian12.Notice(`Created ${records.length} candidate records; zero admissions.`);
    } });
    this.addCommand({ id: "canonize-complete", name: "Run complete canonization", editorCallback: async (_editor, view) => {
      if (view.file)
        await this.canonizationClient.canonizeFile(view.file, ["discovery", "classification", "reconciliation", "claims", "definitions", "mathematics", "theology", "bridges", "defeaters", "review"]);
      new import_obsidian12.Notice("Complete candidate-stage packet created; not admitted.");
    } });
    this.addCommand({ id: "canonize-selected-stages", name: "Run selected canonization stages", editorCallback: async (_editor, view) => {
      if (view.file)
        await this.canonizationClient.canonizeFile(view.file, ["discovery", "classification", "reconciliation"]);
      new import_obsidian12.Notice("Selected candidate stages created; not admitted.");
    } });
    ...
    this.addCommand({ id: "export-candidate-json", name: "Export candidate JSON", editorCallback: async (_editor, view) => {
      ...
      new import_obsidian12.Notice("Candidate JSON validated and exported to Canonization/records.");
    } });
    this.addCommand({ id: "import-reviewed-json", name: "Import reviewed JSON", editorCallback: async (_editor, view) => {
      if (view.file) {
        ...
        new import_obsidian12.Notice(`Imported reviewed candidate ${record.recordId}; not admitted.`);
      }
    } });
    this.addCommand({ id: "refresh-candidate-markdown", name: "Refresh candidate Markdown from JSON", editorCallback: async (_editor, view) => {
      ...
      new import_obsidian12.Notice("Candidate Markdown projection refreshed from governed JSON.");
    } });
    this.addCommand({
      id: "classify-note",
    ...
    this.addCommand({
      ...
      callback: async () => {
        ...
        new import_obsidian12.Notice(this.settings.showHiddenTags ? "Tag blocks shown" : "Tag blocks hidden");
      }
    });
    this.addCommand({
    ...
    this.addCommand({
      ...
      callback: async () => {
        ...
        if (folder) {
          ...
          new import_obsidian12.Notice("Open a note first, so there is a folder to work on.");
        }
      }
    });
    this.addCommand({
      ...
      callback: async () => {
        ...
        if (folder) {
          ...
          new import_obsidian12.Notice("Open a note first, so there is a folder to index.");
        }
      }
    });
    this.addCommand({
      ...
      callback: async () => {
        ...
        try {
          ...
          new import_obsidian12.Notice(`Saved ${markdownPath}, ${jsonPath}, ${reportPath}, ${graphPaths.markdownPath}, ${graphPaths.jsonPath} (${graphCount} notes)`);
          await this.openConceptTracker();
        } catch (error) {
          new import_obsidian12.Notice(`Index export failed: ${this.errorMessage(error)}`);
        }
      }
    });
    this.addCommand({
...
${typeBreakdown}`,
    ...
    this.addCommand({
      ...
      callback: async () => {
        ...
        if (this.app.vault.getAbstractFileByPath(filename)) {
          new import_obsidian12.Notice(`${filename} already exists. Delete or rename it first.`);
          return;
        }
        await this.app.vault.create(filename, this.conceptRegistry.exportJSON());
        new import_obsidian12.Notice(`Exported to ${filename}`);
      }
    });
    this.addCommand({
      id: "save-registry",
      ...
      callback: async () => {
        await this.conceptRegistry.save();
        new import_obsidian12.Notice("Concept registry saved");
      }
    });
    this.addCommand({
      ...
      editorCallback: async (_editor, view) => {
        if (!view.file) {
          new import_obsidian12.Notice("No note is open.");
          return;
        }
        const object = await this.supraInfraqueGraph.registerNote(view.file);
        new import_obsidian12.Notice(`Registered ${object.humanCode} as a proposed candidate; source note unchanged.`);
      }
    });
    this.addCommand({
      ...
      callback: async () => {
        ...
        const folder = (_a = this.app.workspace.getActiveFile()) == null ? void 0 : _a.parent;
        if (!folder) {
          new import_obsidian12.Notice("Open a note first, so there is a folder to export.");
          return;
        }
        const paths = await writeGraphExports(this.app.vault, this.supraInfraqueGraph, folder.path);
        new import_obsidian12.Notice(`Saved ${paths.markdownPath} and ${paths.jsonPath}`);
      }
    });
    this.addCommand({
  ...
  async runClassifierWithTypes(file, types) {
    ...
    if (!validation.valid) {
      new import_obsidian12.Notice(`Cannot classify: ${validation.error}. Check the plugin settings.`);
      return;
    }
    const notice = new import_obsidian12.Notice("Classifying\u2026", 0);
    try {
      const content = await this.app.vault.cachedRead(file);
      const result = await this.classifier.classify(content, types, file.path);
      notice.hide();
      if (result.tags.length === 0) {
        new import_obsidian12.Notice("Nothing matched the selected categories.");
        return;
      }
      this.showResult(file, result.tags, () => this.settings.autoGenerateMermaid);
    } catch (error) {
      notice.hide();
      new import_obsidian12.Notice(this.errorMessage(error), 1e4);
    }
  }
  async classifyAs(file, type) {
    ...
    try {
      ...
      notice.hide();
      if (result.tags.length === 0) {
        new import_obsidian12.Notice(`No ${this.promptManager.getTagTypeName(type).toLowerCase()} found.`);
        return;
      }
  ...
  async runCustomClassifier(file, keyword) {
    ...
    const notice = new import_obsidian12.Notice(`Running "${keyword}"\u2026`, 0);
    try {
      const content = await this.app.vault.cachedRead(file);
      const result = await this.classifier.classifyCustom(content, keyword, file.path);
      notice.hide();
      if (result.tags.length === 0) {
        new import_obsidian12.Notice(`"${keyword}" found nothing.`);
        return;
      }
      this.showResult(file, result.tags, () => false);
    } catch (error) {
  ...
  showResult(file, tags, wantsDiagram) {
    new ClassificationResultModal(
      ...
      async () => {
        ...
        new import_obsidian12.Notice(`Applied ${tags.length} tag${tags.length === 1 ? "" : "s"}`);
  ...
  async regenerateGraph(file) {
    ...
    const tags = await readTags(this.app.vault, file);
    if (tags.length === 0) {
      new import_obsidian12.Notice("This note has no tags yet.");
      return;
    }
    if (this.settings.mermaidPosition === "panel") {
      await this.openSemanticMap(file);
  ...
  async batchClassifyFolder(folder) {
    ...
    if (files.length === 0) {
      new import_obsidian12.Notice("No markdown notes in that folder.");
      return;
    }
  ...
  async runIndexing(scope, folderPath) {
    ...
    try {
      ...
      new import_obsidian12.Notice(`Supra Infraque exported ${graphCount} note${graphCount === 1 ? "" : "s"} to ${graphPaths.markdownPath}; report: ${reportPath}`);
      if (this.settings.enablePostgresSync)
        await this.syncSupraInfraqueGraph();
      await this.openConceptTracker();
    } catch (error) {
      progressModal.close();
      new import_obsidian12.Notice(`Indexing failed: ${this.errorMessage(error)}`);
    }
  }
  ...
  async syncSupraInfraqueGraph() {
    ...
    if (!serviceUrl || !(connection == null ? void 0 : connection.connectionString)) {
      new import_obsidian12.Notice("Set the PostgreSQL helper URL and an active connection first.");
      return;
    }
    try {
      ...
      if (response.status !== 200 || !(result == null ? void 0 : result.success)) {
        new import_obsidian12.Notice(`Graph sync failed: ${(result == null ? void 0 : result.detail) || (result == null ? void 0 : result.error) || `HTTP ${response.status}`}`);
        return;
      }
      new import_obsidian12.Notice(`Graph sync complete: ${result.recordsUpserted || 0} records.`);
    } catch (error) {
      new import_obsidian12.Notice(`Could not reach PostgreSQL helper: ${this.errorMessage(error)}`);
    }
  }
  async buildBridgeDossierProposal(file) {
    ...
    const validation = this.classifier.validateConfiguration();
    if (!validation.valid) {
      new import_obsidian12.Notice(`Cannot build bridge dossier: ${validation.error}.`);
      return;
    }
    try {
      ...
      new import_obsidian12.Notice(`Saved neutral bridge dossier proposal: ${outputPath}`);
    } catch (error) {
      new import_obsidian12.Notice(`Bridge dossier failed: ${this.errorMessage(error)}`);
    }
  }
  async showFolderSelectionForIndex() {
    const folders = this.app.vault.getAllLoadedFiles().filter((f) => f instanceof import_obsidian12.TFolder);
  ...
  async revealLeaf(viewType) {
    ...
    if (!leaf) {
      new import_obsidian12.Notice("Could not open the side panel.");
      return null;
    }
  ...
  async generateConceptForwardLinks(journey) {
    if (journey.occurrences.length < 2) {
      new import_obsidian12.Notice("A concept needs at least two occurrences to link them up.");
      return;
    }
    let linksAdded = 0;
...
%%forward-link::${journey.concept}::[[${next.fileName}]]%%`;
    ...
    new import_obsidian12.Notice(`Added ${linksAdded} forward link${linksAdded === 1 ? "" : "s"} for "${journey.concept}"`);
  }
};
