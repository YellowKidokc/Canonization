...
import { COMPLETE_STAGE_RUN, orderStages, StageId } from "../engine/stages";
import { FakeProviderResponse, mapFakeProviderResponse } from "../engine/provider-mapping";
...
export class CanonizationClient {
  ...
  async canonizeFile(file: TFile, stages: StageId[] = ["discovery","classification","reconciliation"], fakeResponse?: FakeProviderResponse): Promise<CanonizationRecord> {
    ...
    let record=createCandidateRecord({sourceId:file.path,sourceHash:hash,coordinates:{vaultPath:file.path,mtime:file.stat.mtime},actor:"canonization",blindResult:{stage:"discovery",sourceContentHash:hash,objects:[],open:["AI stage results pending or imported separately"]},now});
    record.source.title=file.basename; record.source.uri=`obsidian://open?file=${encodeURIComponent(file.path)}`; record.provenance.push({event:"candidate-created",at:now,client:"canonization",sourceModified:false,admissionPerformed:false});
    if(fakeResponse) record=mapFakeProviderResponse(record,fakeResponse,now);
