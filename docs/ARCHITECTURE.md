...
Canonization reads selected notes/folders and performs configured candidate stages. `CanonizationClient` emits governed JSON plus readable Markdown under a vault-relative `Canonization` folder at runtime. The external workbench imports the same JSON, preserves unknown fields, locks protected discovery and authority fields, autosaves a recoverable browser draft, and exports reviewed JSON. Obsidian validates that JSON before regenerating Markdown.
