...
var EPISTEMIC_PRESET = {
  ...
  categories: [
    ...
    category(
      ...
      8,
      "Identify independently gradable candidate-canonization objects. Return exact_claim, proposed_text, target_canon_id, target_section, source file/span, object type, register, warrant, dependencies, support, defeat conditions, OPEN fields, target lane, and truth predicates. Preserve UNKNOWN; bridges never transfer proof; no admission."
    ),
    ...structuralCategories()
  ],
  topics: [
...
var LEAN_PRESET = {
  ...
  categories: [
    ...
    category("Canonization", "Canonization", "Canonization candidates", 8, "Identify independently gradable objects for candidate canonization. Return exact_claim, proposed_text, target_canon_id, target_section, proposed object type, register, warrant, source span, dependencies, support, defeat conditions, OPEN fields, target lane, and truth_predicates (truth mode, premise dependence, necessity, reversibility, invariants, countermodels, Why-Closure, closure level). Preserve UNKNOWN. Never overwrite human-authored text or convert Lean, physics, bridge, or theological material into admission."),
    ...structuralCategories()
  ],
  topics: [
...
Also assign each element the ${this.axisLabel().toLowerCase()} values that apply.` : "";
    const metadataInstruction = type === "Canonization" ? `
- "metadata": an object containing exact_claim, proposed_text, target_canon_id, target_section, source_file, source_span, proposed_object_type, register, warrant, dependencies (array), support (array), defeat_conditions (array), open_fields (array), proposed_lane, and truth_predicates. truth_predicates must contain truth_mode, premise_dependency, necessity, reversibility, invariants (array), countermodels (array), why_closure, and closure_level. Use UNKNOWN rather than guessing.` : "";
...
- "confidence": (optional) a score from 0 to 1${metadataInstruction}${this.topicField()}${this.topicSection()}
...
var AIClassifier = class {
  ...
  async classifySingleType(content, type, sourceFile) {
    ...
    return {
      ...
      // Kept in memory for caller-side diagnostics. Canonization Workbench
      // writes only a bounded excerpt when a zero-result run fails closed.
      rawResponse: response,
      parsedResponseCount: parsed.length,
  ...
  parseAIResponse(response) {
    ...
    try {
      ...
      // Canonization prompts sometimes return a named envelope such as
      // { "proposals": [...] } instead of the legacy bare array.  Flatten
      // those envelopes here so valid candidate objects are not silently
      // discarded by convertToTags.
      const unpack = (value) => {
        if (Array.isArray(value)) return value.flatMap(unpack);
        if (!value || typeof value !== "object") return [];
        const containerKeys = ["proposals", "candidates", "objects", "elements", "results", "tags", "discovered_objects"];
        for (const key of containerKeys) {
          if (Array.isArray(value[key])) return value[key].flatMap(unpack);
        }
        return [value];
      };
  ...
  convertToTags(responses, sourceFile) {
    ...
    for (const response of responses) {
      ...
      const directCanonFields = [
        "exact_claim", "proposed_text", "target_canon_id", "target_section",
        "source_file", "source_span", "proposed_object_type", "register",
        "warrant", "dependencies", "support", "defeat_conditions",
        "open_fields", "proposed_lane", "truth_predicates", "truth_statements",
        "bfp_dg", "exact_negation", "countermodels", "discriminating_tests",
        "contradiction_hooks"
      ];
      const hasCanonShape = directCanonFields.some((key) => Object.prototype.hasOwnProperty.call(response, key));
      const isBlindDiscovery = typeof response.discovery_id === "string" && (typeof response.exact_quotation === "string" || typeof response.neutral_paraphrase === "string");
      const type = response.type || (hasCanonShape || isBlindDiscovery ? "Canonization" : null);
      const label = response.label || response.exact_claim || response.proposed_text || response.title || response.name || response.exact_quotation || response.neutral_paraphrase || response.discovery_id;
      if (!type || !label) continue;
      const metadata = response.metadata && typeof response.metadata === "object" ? { ...response.metadata } : {};
      for (const key of directCanonFields) {
        if (metadata[key] === void 0 && response[key] !== void 0) metadata[key] = response[key];
      }
      if (isBlindDiscovery) {
        if (metadata.exact_claim === void 0) metadata.exact_claim = response.exact_quotation || label;
        if (metadata.proposed_text === void 0) metadata.proposed_text = response.neutral_paraphrase || response.exact_quotation || label;
        if (metadata.source_span === void 0) metadata.source_span = `discovery:${response.discovery_id}`;
        if (metadata.proposed_object_type === void 0) metadata.proposed_object_type = "OPEN";
        if (metadata.register === void 0) metadata.register = "OPEN";
        if (metadata.warrant === void 0) metadata.warrant = "UNKNOWN";
        if (metadata.proposed_lane === void 0) metadata.proposed_lane = "proposal";
        if (metadata.defeat_conditions === void 0) metadata.defeat_conditions = Array.isArray(response.collapse_conditions) ? response.collapse_conditions : [];
        if (metadata.open_fields === void 0) metadata.open_fields = Array.isArray(response.open_questions) ? response.open_questions : ["Human review required"];
        if (metadata.truth_predicates === void 0) metadata.truth_predicates = {};
      }
      const tag = createTag(
        type,
        String(label),
        null,
        type === "Custom" ? response.customType : void 0,
        sourceFile
      );
      if (Object.keys(metadata).length > 0) tag.metadata = metadata;
      if (wantTopics) {
      ...
      labelToUuid.set(String(label), tag.uuid);
    }
